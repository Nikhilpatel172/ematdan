<?php 
    ...
    $sql1 = "SELECT * FROM `result` having `result`.`novotes` = (Select `result`.`novotes` from result ORDER BY `result`.`novotes` DESC Limit 1)";
    $retval =  mysqli_query( $conn , $sql1);
    if(! $retval ) {
       die('Could not display declarED results ' . mysqli_error($conn));
    }
    else{
        if(mysqli_affected_rows($conn) ==  1)
        {
            ...
            $NAME = $row['name'];
            $sql = "UPDATE result SET winner = 'YES' where name='$NAME';";
            $retval =  mysqli_query( $conn , $sql);
            if(! $retval ) {
               die('Could not display declarED Winner ' . mysqli_error($conn));
            }
        }
        else{
            ?>
    ...
    <nav class="navbar navbar-toggleable-md bg-primary fixed-top navbar-transparent " color-on-scroll="1">
        <div class="container">
            <div class="navbar-translate">
                ...
                <a class="navbar-brand" href="#" rel="tooltip" title="Designed and Developed by Nikhil & Kushal" data-placement="bottom" target="">
                    e-Matdan
                </a>
            </div>
    ...
    <div class="wrapper">
        <div class="page-header clear-filter" filter-color="green">
            ...
            <div class="container" style="overflow-y:scroll;">
                ...
                <a href="admin_candidate.php"><input class="btn btn-primary btn-round" type="submit" name="View" value="Go to dashboard" /></a>
